...
func NewConnectionStore(s fyne.Storage) (*ConnectionStore, error) {
	store := &ConnectionStore{
		...
		savedItems:  make(SavedConnections),
		storage:     s,
	}
...
// GetDeviceSaved retrieves saved connections from disk
func (cs *ConnectionStore) GetDeviceSaved(deviceID string) ([]client.RemoteAccessTarget, bool) {
	cs.mutex.Lock()
	defer cs.mutex.Unlock()
...
// SetSaved sets saved connections in memory
func (cs *ConnectionStore) SetSaved(connections SavedConnections) {
	cs.mutex.Lock()
	defer cs.mutex.Unlock()
	cs.savedItems = connections
}

// SaveOnConnect saves connections to disk when a device connects
func (cs *ConnectionStore) SaveOnConnect(deviceID string, targets []client.RemoteAccessTarget) error {
	cs.mutex.Lock()
	cs.savedItems[deviceID] = targets
	cs.mutex.Unlock()
	return cs.SaveToDisk()
}

// GetSaved retrieves all saved connections
func (cs *ConnectionStore) GetSaved() SavedConnections {
	cs.mutex.Lock()
	defer cs.mutex.Unlock()
	return cs.savedItems
}

// SaveToDisk saves connections to disk
func (cs *ConnectionStore) SaveToDisk() error {
	// Unlock before IO
